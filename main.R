...
punt_directa = apply(na.omit(datos_dico), 1, sum) # Calcular puntuación directa
...
media1.var1.g1 = coef(object = extract.group(m1,1))$GroupPars; media.var.g1
media1.var1.g2 = coef(object = extract.group(m1,2))$GroupPars; media.var.g2
...
test_anclaje = par.g1[which(m1_libre$adj_pvals>.05),] # Seleccionar ítems sin DIF en el primer paso.
mayor_a = order(test_anclaje[,1],decreasing=TRUE)[1:5] # Seleccionar los más discriminativos.
# test_anclaje[order(test_anclaje[,1],decreasing=TRUE),]
...
z = rnorm(1000, 0, 1)
I = length(z)
respuestas_simuladas = catR::genPattern(z, params)
respondents = catR::simulateRespondents(thetas = z,
                                        itemBank = params,
                                        start = inicio,
                                        test = test,
                                        stop = parada,
...
plot(respondents)
catR::print.catResult(respondents)
psych:::describe(respondents$exposureRates)
psych:::describe(respondents$final.values.df)
psych:::describe(respondents$exposureRates)

save.image("DATOSLEIDOS.RDATA")
